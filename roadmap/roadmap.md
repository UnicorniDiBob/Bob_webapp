...
| AREA1 | Area personale: LAVORO e IMPOSTAZIONI separati (revisione di Lucio del 19/08 pomeriggio - "sono 2 robe diverse"). Nella prima versione tutto stava sotto /dashboard e la barra laterale metteva "Oggi" accanto a "Accesso e sicurezza": il lavoro di ogni giorno sulla stessa fila di cose che si aprono due volte l'anno. Ora /dashboard e' SOLO lavoro (pro: richieste, calendario, giornata; cliente: lavori in corso, appuntamenti, e un bottone per cercare un professionista) e le 9 sezioni di configurazione vivono sotto /impostazioni con un guscio proprio, che in cima ha il ritorno al lavoro. HEADER: via l'etichetta "Account", identica per tutti e muta su cosa contenesse; il bottone visibile porta al lavoro e dipende dal ruolo ("Il mio lavoro" / "I miei lavori"), la configurazione e' un'icona UserCog accanto - non Settings, che nello stesso header e' gia' il pannello Admin. Quattro redirect 307 in next.config.mjs, con i due casi speciali PRIMA del jolly. Aggiunti /impostazioni a robots.ts e a ProBanner. TROVATO CON LA PROVA SULLE ROTTE, non leggendo il file: il jolly scritto :sezione* combaciava anche con ZERO segmenti, quindi /dashboard nudo rimbalzava su /impostazioni e l'area di lavoro era irraggiungibile - corretto in :sezione+. RESTA PER CHIUDERLA: controllo dal vivo dopo il deploy (desktop e 390px), con attenzione al bottone del ruolo nel header e al ritorno dal guscio delle impostazioni. | Client/Pro | Claude | 🔵 In corso | 2026-08-19 → 2026-08-31 |
...
**Finestra:** 2026-09-01 → 2026-11-30 · **10 aperte, 2 chiuse**
...
| PWD2 | Doppia password all'iscrizione. Il form aveva UN solo campo e nessun controllo di lunghezza lato client, con placeholder e minLength ancora a 6 mentre Supabase rifiuta sotto gli 8 dal 9 agosto: chi sbagliava a digitare si registrava con una password che non conosceva e l'unica uscita era 'password dimenticata'. Funziona - quella mail la manda Supabase, non Resend - ma e' un giro che non deve servire, e a ottobre lo farebbe un professionista appena reclutato al telefono. Ora: secondo campo in iscrizione, avviso immediato se non coincidono, soglia PASSWORD_MIN=8 in una costante sola. CONDIZIONE PER CHIUDERLA: push, deploy e una prova di iscrizione vera. | Client/Pro | Claude | 🔵 In corso | 2026-08-19 → 2026-08-26 |
| CANC1 | Cancellazione account: scritta e provata, IN ATTESA DI DEPLOY. Condizioni per chiuderla: (1) push e deploy; (2) CRON_SECRET e' gia' in produzione ma il nuovo cron /api/cron/cancella-account e' registrato in vercel.json solo da questo commit, quindi il primo giro va verificato in system_job_runs - questa volta la riga si scrive SEMPRE, anche a vuoto e in errore, proprio per non ripetere gli undici semafori verdi di verifica-piva; (3) una prova end-to-end vera: chiedere la cancellazione di un account di test, controllare che sparisca dagli elenchi, annullare, ricontrollare che torni. AGGIORNAMENTO 25/08: "in attesa di deploy" era vero solo a meta', perche' non c'era niente da deployare. Il codice non era mai stato committato: dal 19 al 25 agosto la 056 e' stata in produzione mentre le 996 righe che la usano non stavano in nessun commit e su nessun ramo (git log --all --grep=cancellazione: zero risultati) - vivevano solo in un file di patch salvato nel progetto Claude. Ripristinate il 25/08 col commit db49be7, verificando una per una le 20 impronte git scritte nel patch: tutte coincidono, quindi il ripristino e' il lavoro del 19/08 byte per byte e non una riscrittura. Build e lint puliti; deploy confermato dall'esterno, perche' il testo nuovo dell'informativa e' comparso su www.meetonda.com/privacy. LA LEZIONE, che vale piu' di questa riga: e' la SECONDA volta dopo la 015/016 che una migrazione vive in produzione senza il suo file nel repo, e stavolta si e' portata dietro anche il codice. Il controllo che manca non e' umano: nessuno script confronta la storia delle migrazioni live con supabase/migrations, e finche' non c'e' la prossima deriva la trovera' un'ispezione, non un test. RESTA PER CHIUDERLA: il primo giro del cron in system_job_runs (dopo le 23:00 UTC) e la prova end-to-end vera. | Shared | Claude | 🔵 In corso | 2026-08-19 → 2026-08-26 |
| RET1 | Job di retention (P3.9). Due regole sono DICHIARATE nel ROPA e non ancora APPLICATE da nessun codice, e finche' e' cosi' quelle righe del registro sono incomplete: i ticket di assistenza anonimi (support_tickets con user_id null) vanno cancellati dopo 12 mesi, e i dati di prospect/waitlist idem. Il cron della cancellazione account e' il posto naturale dove aggiungerle: gira ogni notte e ha gia' il service role e la registrazione del giro. | Internal | Lucio | ⬜ Aperto | 2026-10-01 → 2026-10-31 |
...
| G08 | Cancellazione account self-service, e la de-identificazione delle recensioni che la bloccava (mig 056). IL DISEGNO VIENE DALLE FONTI, non dal buon senso, e su un punto ha corretto la richiesta iniziale: il motivo NON puo' essere obbligatorio, perche' l'art. 12(2) GDPR obbliga ad AGEVOLARE l'esercizio dei diritti e pretendere una motivazione e' un ostacolo. I 7 giorni stanno dentro l'art. 12(3) e non sono un ritardo ingiustificato ex art. 17(1) A CONDIZIONE che siano dichiarati: lo sono in /impostazioni/accesso, nell'avviso in cima a ogni pagina e nell'informativa. La terza cosa e' quella che ha cambiato il disegno: durante l'attesa il profilo si SPEGNE (professionals.deactivated_at, nuovo), altrimenti staremmo rimandando una cancellazione mentre continuiamo a trattare i dati. G16 CHIUSA DI CONSEGUENZA: ratings.customer_id era NOT NULL con ON DELETE CASCADE (mig 012), quindi cancellare un cliente cancellava le sue recensioni - ora va a NULL, e il profilo pubblico non mostrava comunque il nome dell'autore, quindi nessuna modifica di interfaccia. Chiuso anche il punto 8 del ROPA: il cron svuota verifica-documenti/<user_id>/ prima di cancellare. PROVATO con 7 controlli su un utente non-staff: la recensione sopravvive de-identificata, la richiesta va via per cascata, il motivo anonimo resta, non si vede la richiesta di altri, non si inserisce dal client, si annulla la propria. Righe RoPA A20 e A6 aggiornate, informativa aggiornata. | Claude | 2026-08-19 |
