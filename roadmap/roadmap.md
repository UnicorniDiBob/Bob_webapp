...
036 and 037 were applied in production without repo files. Migration 038 was written to be self-sufficient and idempotent, so a fresh clone reproduces production correctly — the drift rule worked. What remains is cosmetic but affects tooling: five names in the live history have no matching file (portfolio_pro_limit_1, 034b, 034c, 036, 037), 011_customer_memory has a file but no history row, and numbering has collided three times (029, 034 x3, 042 x2). Until that is reconciled, `supabase db diff` cannot be trusted as a check.
